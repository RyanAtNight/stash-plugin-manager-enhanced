import { version as pluginManagerVersion } from "../package.json";
import {
...
const GITHUB_BUTTON_CONTENT = `<svg class="spme-github-icon" viewBox="0 0 16 16" width="19" height="19" aria-hidden="true" focusable="false"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82A7.65 7.65 0 0 1 8 3.87c.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0 0 16 8c0-4.42-3.58-8-8-8Z"></path></svg>`;
...
const TRASH_ICON = `<svg class="spme-trash-icon" viewBox="3 3 18 18" width="19" height="19" aria-hidden="true" focusable="false"><path d="M4 7h16"></path><path d="M9 7V4h6v3"></path><path d="m6 7 1 13h10l1-13"></path><path d="M10 11v5M14 11v5"></path></svg>`;

function githubRepositoryLink(url, name) {
  const label = `Open ${name} GitHub repository`;
  return `<a class="spme-repo-link spme-icon-action" href="${escapeHTML(url)}" target="_blank" rel="noopener noreferrer" aria-label="${escapeHTML(label)}" title="${escapeHTML(label)}">${GITHUB_BUTTON_CONTENT}</a>`;
}
...
export class EnhancedPluginManager {
  ...
  updateActionHTML(pkg) {
    if (pkg.status !== "update") return "";
    return `<button type="button" data-action="update-one" data-id="${escapeHTML(pkg.package_id)}">Update</button>`;
  }
